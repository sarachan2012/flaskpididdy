...
def get_audio_lowest_refetch(image_id, audio_id):
    ...
    return (db.session.query(Audio).filter(Audio.image_id==image_id).filter(Audio.audio_id==audio_id)).order_by(Audio.refetch).get(1)
    # return (db.session.query(Audio).filter(Audio.image_id==image_id).filter(Audio.audio_id==audio_id)).order_by(Audio.refetch).limit(1).all()

def get_audio_lowest_refetch_image_only(image_id):
    # order by asc and first
    return db.session.query(Audio).filter(Audio.image_id==image_id).order_by(Audio.refetch).get(1)